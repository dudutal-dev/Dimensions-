...
function toDraft(event: EveningEvent): EventDraft {
  const unit: Unit =
    event.recoveryMin >= 1440 && event.recoveryMin % 1440 === 0
      ? "day"
      : event.recoveryMin >= 60 && event.recoveryMin % 60 === 0
        ? "hour"
        : "min";
  return {
    what: event.what,
    body: event.body,
    did: event.did,
    amount: String(event.recoveryMin / UNIT_MINUTES[unit]),
    unit,
    dim: event.dim,
  };
}
...
function toEvent(draft: EventDraft): EveningEvent | null {
  ...
  return {
    ...
    did: draft.did.trim(),
    recoveryMin: amount * UNIT_MINUTES[draft.unit],
    dim: draft.dim,
  };
}
...
export function EveningPage() {
  ...
  const date = /^\d{4}-\d{2}-\d{2}$/.test(params.date ?? "")
    ...
    : isoDate(new Date());
  return (
    <EveningEditor key={date} date={date} onBack={() => navigate("/journal")} />
  );
}

function EveningEditor({ date, onBack }: { date: string; onBack: () => void }) {
  const saved = useSavedFlash();
  const { value, setValue, loaded } = useDraft<{ events: EventDraft[] }>(
    `evening:${date}`,
    { events: [emptyEvent()] },
  );
  ...
  useEffect(() => {
    ...
    void Promise.all([
      repos.drafts.load(`evening:${date}`),
      repos.evenings.getByDate(date),
    ]).then(([draft, entry]) => {
      if (!draft && entry?.events.length)
        setValue({ events: entry.events.map(toDraft) }, { immediate: true });
    });
  ...
  const commit = (events: EventDraft[], immediate: boolean) => {
    ...
    void tracked(repos.evenings.saveForDate(date, complete)).catch(
      () => undefined,
    );
  };
  const patch = (
    index: number,
    change: Partial<EventDraft>,
    immediate = false,
  ) =>
    commit(
  ...
  return (
    <>
      ...
      <p className="mt-2 text-muted">
        עד שלושה אירועים טעונים מהיום. לא חייבים את הגדולים ביותר — רק את מה
        שעדיין מורגש.
      </p>
              ...
              <Card padding="lg" className="flex flex-col gap-4">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {value.events.length > 1 && (
                    <IconButton
                      label={`מחיקת אירוע ${index + 1}`}
                      onClick={() =>
                        commit(
                          value.events.filter((_, i) => i !== index),
                          true,
                        )
                      }
                    >
                ...
                <TextArea
                  ...
                  rows={2}
                  value={event.what}
                  onChange={(e) => patch(index, { what: e.target.value })}
                />
                <TextField
                  label="מה הרגשתי בגוף?"
                  value={event.body}
                ...
                <TextField
                  label="מה עשיתי?"
                  value={event.did}
                  onChange={(e) => patch(index, { did: e.target.value })}
                  autoComplete="off"
                />
                <div>
                  <div className="flex flex-wrap items-end gap-3">
                    <div className="w-24 shrink-0">
                      <TextField
                        ...
                        value={event.amount}
                        onChange={(e) =>
                          patch(index, { amount: e.target.value })
                        }
                        autoComplete="off"
                        aria-invalid={amountInvalid(event) || undefined}
                        className={
                          amountInvalid(event) ? "border-danger" : undefined
                        }
                      />
                    </div>
                    ...
                    {DIMS.map((dim) => (
                      ...
                      >
                        <DimensionGlyph
                          dim={dim}
                          size={26}
                          variant={event.dim === dim ? "solid" : "line"}
                          decorative
                        />
                ...
                <p className="text-xs text-muted">
                  {recorded
                    ? "האירוע רשום ביומן."
                    : "האירוע יירשם כשיהיו בו מה קרה, זמן עד איזון וסיווג. עד אז הוא שמור כטיוטה."}
                </p>
              </Card>
            </li>
          );
      ...
      {value.events.length < MAX_EVENING_EVENTS && (
        <Button
          ...
          onClick={() => commit([...value.events, emptyEvent()], true)}
        >
          עוד אירוע
        </Button>
      )}
